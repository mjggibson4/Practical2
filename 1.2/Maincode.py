...
Creation of a suitable GUI to input user parameters
Web scrape initial bombing location found
Was found through testing that an infinite loop could be entered via an 
inappropriate prob_up value. Dealt with.
Particle movement ceased at boundary values
"""
...
def move_bacteria(bomb_origin, num_bact, elevation, wind_probabilities, heatmap):
    ...
    for i in range(num_bact):
        ...
        while bacteria[i].elevation != 0 and bacteria[i].hangtime != 1000 and bacteria[i].bomb_origin_y != 0 and bacteria[i].bomb_origin_x != 0 and bacteria[i].bomb_origin_y != 300 and bacteria[i].bomb_origin_x != 300:  
        ...
        # Deal with scenario of infinite loop:
        ...
        if bacteria[i].elevation == 0:    
            ...
            print('Help')
    print('tester', bact_counter/num_bact)        
...
def calc_distances(bomb_origin, agents):
    ...
    for i in range(len(agents)):
        ...
        - agents[i].bomb_origin_x)**2)**0.5
        distances.append(dist)
        #print(len(distances))
    return distances


def calc_axis_distances(bomb_origin, agents):
    """ Function to calculate the distance each bacteria has travelled from 
    bomb origin
    ...
    TODO: Not working quite as expected    
    """    
    axis_distances = [50,150]
    for i in range(len(agents)):
        if abs(bomb_origin[1] - agents[i].bomb_origin_y) > abs(axis_distances[1]):
            axis_distances[1] = agents[i].bomb_origin_y - bomb_origin[1] 
        if abs(bomb_origin[0] - agents[i].bomb_origin_x) > abs(axis_distances[0]):
            axis_distances[0] = agents[i].bomb_origin_x - bomb_origin[0]
    return axis_distances   
...
def run(canvas, num_bact, elevation, prob_north, prob_west, prob_east, prob_south, prob_up, prob_level, prob_down):
    ...
    heatmap = create_blank_heatmap(len(enviroment)+1)
    ...
    y = calc_axis_distances(bomb_origin, bacteria)
    print(y)
    times = []
