...
This script constructs a User Interface from which a bacterial plume model
is run. The GUI contains two menus: model and help. This GUI allows users to 
change the initial conditions of the model and calls the script maincode
which contains the code by which the model is actually run.
...
def launch_gui():
    """Launches master GUI window from which the Bacterial model can be run.
 
    Launces a GUI in which the user can edit the initial conditions of the
    bacterial bomb model and run a simulation. This GUI hosts a canvas in which 
    the final resting locations of the bacteria can be observed.
    ...
    Returns:
        ...
        interact with a run the bacterial model:
            
            Cascade Menu - Run:
                Run - Commands program to run the bacterial bomb model using the
                parameters defined.
                
                Set Parameters - Calls the function SetParameters() in which
                the default values of num_bact, elevation, prob_north, 
                prob_west, prob_east, prob_south, prob_up, prob_level, 
                prob_down can all be redefined.
            ...
            Cascade Menu: Help
                ...
                associated with the bacterial bombon model,
    ...
    root = tk.Tk()
    root.wm_title("Bacterial Bomb Model")
    ...
    # Use lamba functions in order to stop commands running automatically 
    # without prompt
...
def set_parameters():
    """Creates a GUI in which users can redefine the initial model conditions.

    Creates a GUI in which the model variables num_bact, elevation, prob_north, 
    prob_west, prob_east, prob_south, prob_up, prob_level, 
    prob_down. These variables are validated and 
    redefined within the pass_entry_fields() function
    ...
    def pass_entry_fields():
        """Validates user input from Set_Parameter function.
        
        Takes the values input into the set parameters functions and checks
        ...
        * The parameters defining the number of bacteria and elevation must be
          positive integers

        
        Args:
            e1: temp variable for prob_north
            e2: temp variable for prob_west
            e3: temp variable for prob_east
            e4: temp variable for prob_south
            e5: temp variable for prob_up
            e6: temp variable for prob_level
            e7: temp variable for prob_down
            e8: temp variable for
            e9: temp variable for
        ...
        Raises:
            Error Box: "All values must be positive integers"
            Error Box: "Probabilities must add to 1"
            Error Box: "Values must be numeric"
        ...
        try:
            ...
            # Construct xy probabilites into a list
            xy_prob = [float(e3.get()), float(e4.get()), float(e5.get()),
                       float(e6.get())]
            # Construct z probabilitiese into a list
            z_prob = [float(e7.get()), float(e8.get()), float(e9.get())]
            
            # If all xy and z probabilities sum to 1 and are positive reassign 
            # values
            ...
            # If elevation and number of bacteria are postive whole number
            # reassign values                       
...
def show_about_info():
# Function to show rudimental information regarding bacterial bomb model   
    messagebox.showinfo("About", "Bacterial Bomb Model \n\n" "Author: Michael Gibson\n" 
                        "Version: 1.4\n" "Released: Jan, 2020")
 
    
def show_help_info():
# Function to show help information regarding bacterial bomb model
    messagebox.showinfo("Help", "Instructions on running this program and troubleshooting tips can be found here:\n" 
                        "https://github.com/mjggibson4/Practical2/tree/master")
